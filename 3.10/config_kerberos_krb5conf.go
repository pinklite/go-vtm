...
func (vtm VirtualTrafficManager) ListKerberosKrb5Confs() (*[]string, *vtmErrorResponse) {
	conn := vtm.connector.getChildConnector("/tm/3.10/config/active/kerberos/krb5confs")
	data, ok := conn.get()
	if ok != true {
...
func (vtm VirtualTrafficManager) GetKerberosKrb5Conf(name string) (string, *vtmErrorResponse) {
	...
	conn := vtm.connector.getChildConnector("/tm/3.10/config/active/kerberos/krb5confs/" + name)
	data, ok := conn.get()
	if ok != true {
		object := new(vtmErrorResponse)
		json.NewDecoder(data).Decode(object)
