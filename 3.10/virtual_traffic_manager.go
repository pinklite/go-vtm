...
func (c vtmConnector) getChildConnector(path string) *vtmConnector {
	newUrl := c.url + path
	conn := newConnector(c.client, newUrl, c.username, c.password, c.verifySslCert, c.verbose)
	return conn
}
...
func newClient(verifySslCert bool) *http.Client {
	return &http.Client{
...
func newConnector(client *http.Client, url, username, password string, verifySslCert, verbose bool) *vtmConnector {
	conn := &vtmConnector{
		url:           url,
		username:      username,
		password:      password,
...
func NewVirtualTrafficManager(url, username, password string, verifySslCert, verbose bool) (*VirtualTrafficManager, bool, *vtmErrorResponse) {
	...
	client := newClient(verifySslCert)
	conn := newConnector(client, url, username, password, verifySslCert, verbose)
	vtm.connector = conn
