...
func (vtm VirtualTrafficManager) ListDnsServerZoneFiles() (*[]string, *vtmErrorResponse) {
	conn := vtm.connector.getChildConnector("/tm/3.10/config/active/dns_server/zone_files")
	data, ok := conn.get()
	if ok != true {
...
func (vtm VirtualTrafficManager) GetDnsServerZoneFile(name string) (string, *vtmErrorResponse) {
	...
	conn := vtm.connector.getChildConnector("/tm/3.10/config/active/dns_server/zone_files/" + name)
	data, ok := conn.get()
	if ok != true {
		object := new(vtmErrorResponse)
		json.NewDecoder(data).Decode(object)
