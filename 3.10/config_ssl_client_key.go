...
func (vtm VirtualTrafficManager) GetSslClientKey(name string) (*SslClientKey, *vtmErrorResponse) {
	...
	conn := vtm.connector.getChildConnector("/tm/3.10/config/active/ssl/client_keys/" + name)
	data, ok := conn.get()
	if ok != true {
...
func (vtm VirtualTrafficManager) ListSslClientKeys() (*[]string, *vtmErrorResponse) {
	conn := vtm.connector.getChildConnector("/tm/3.10/config/active/ssl/client_keys")
	data, ok := conn.get()
	if ok != true {
		object := new(vtmErrorResponse)
		json.NewDecoder(data).Decode(object)
