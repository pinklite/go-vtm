...
func (vtm VirtualTrafficManager) ListActionPrograms() (*[]string, *vtmErrorResponse) {
	conn := vtm.connector.getChildConnector("/tm/3.10/config/active/action_programs")
	data, ok := conn.get()
	if ok != true {
...
func (vtm VirtualTrafficManager) GetActionProgram(name string) (string, *vtmErrorResponse) {
	...
	conn := vtm.connector.getChildConnector("/tm/3.10/config/active/action_programs/" + name)
	data, ok := conn.get()
	if ok != true {
		object := new(vtmErrorResponse)
		json.NewDecoder(data).Decode(object)
