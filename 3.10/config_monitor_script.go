...
func (vtm VirtualTrafficManager) ListMonitorScripts() (*[]string, *vtmErrorResponse) {
	conn := vtm.connector.getChildConnector("/tm/3.10/config/active/monitor_scripts")
	data, ok := conn.get()
	if ok != true {
...
func (vtm VirtualTrafficManager) GetMonitorScript(name string) (string, *vtmErrorResponse) {
	...
	conn := vtm.connector.getChildConnector("/tm/3.10/config/active/monitor_scripts/" + name)
	data, ok := conn.get()
	if ok != true {
		object := new(vtmErrorResponse)
		json.NewDecoder(data).Decode(object)
