...
func (vtm VirtualTrafficManager) GetAptimizerScope(name string) (*AptimizerScope, *vtmErrorResponse) {
	...
	conn := vtm.connector.getChildConnector("/tm/3.10/config/active/aptimizer/scopes/" + name)
	data, ok := conn.get()
	if ok != true {
...
func (vtm VirtualTrafficManager) ListAptimizerScopes() (*[]string, *vtmErrorResponse) {
	conn := vtm.connector.getChildConnector("/tm/3.10/config/active/aptimizer/scopes")
	data, ok := conn.get()
	if ok != true {
		object := new(vtmErrorResponse)
		json.NewDecoder(data).Decode(object)
