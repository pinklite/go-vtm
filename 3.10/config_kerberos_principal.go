...
func (vtm VirtualTrafficManager) GetKerberosPrincipal(name string) (*KerberosPrincipal, *vtmErrorResponse) {
	...
	conn := vtm.connector.getChildConnector("/tm/3.10/config/active/kerberos/principals/" + name)
	data, ok := conn.get()
	if ok != true {
...
func (vtm VirtualTrafficManager) ListKerberosPrincipals() (*[]string, *vtmErrorResponse) {
	conn := vtm.connector.getChildConnector("/tm/3.10/config/active/kerberos/principals")
	data, ok := conn.get()
	if ok != true {
		object := new(vtmErrorResponse)
		json.NewDecoder(data).Decode(object)
