...
func (vtm VirtualTrafficManager) ListKerberosKeytabs() (*[]string, *vtmErrorResponse) {
	conn := vtm.connector.getChildConnector("/tm/3.10/config/active/kerberos/keytabs")
	data, ok := conn.get()
	if ok != true {
...
func (vtm VirtualTrafficManager) GetKerberosKeytab(name string) (string, *vtmErrorResponse) {
	...
	conn := vtm.connector.getChildConnector("/tm/3.10/config/active/kerberos/keytabs/" + name)
	data, ok := conn.get()
	if ok != true {
		object := new(vtmErrorResponse)
		json.NewDecoder(data).Decode(object)
