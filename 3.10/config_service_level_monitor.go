...
func (vtm VirtualTrafficManager) GetServiceLevelMonitor(name string) (*ServiceLevelMonitor, *vtmErrorResponse) {
	...
	conn := vtm.connector.getChildConnector("/tm/3.10/config/active/service_level_monitors/" + name)
	data, ok := conn.get()
	if ok != true {
...
func (vtm VirtualTrafficManager) ListServiceLevelMonitors() (*[]string, *vtmErrorResponse) {
	conn := vtm.connector.getChildConnector("/tm/3.10/config/active/service_level_monitors")
	data, ok := conn.get()
	if ok != true {
		object := new(vtmErrorResponse)
		json.NewDecoder(data).Decode(object)
