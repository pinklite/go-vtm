...
func (vtm VirtualTrafficManager) GetPool(name string) (*Pool, *vtmErrorResponse) {
	...
	conn := vtm.connector.getChildConnector("/tm/3.10/config/active/pools/" + name + "?expert_keys=auto_scaling/extraargs")
	data, ok := conn.get()
	if ok != true {
...
func (vtm VirtualTrafficManager) DeletePool(name string) *vtmErrorResponse {
	conn := vtm.connector.getChildConnector("/tm/3.10/config/active/pools/" + name)
	data, ok := conn.delete()
	if ok != true {
		object := new(vtmErrorResponse)
		json.NewDecoder(data).Decode(object)
...
func (vtm VirtualTrafficManager) ListPools() (*[]string, *vtmErrorResponse) {
	conn := vtm.connector.getChildConnector("/tm/3.10/config/active/pools")
	data, ok := conn.get()
	if ok != true {
		object := new(vtmErrorResponse)
		json.NewDecoder(data).Decode(object)
