...
func (vtm VirtualTrafficManager) GetSystemBackupsFull(name string) (*SystemBackupsFull, *vtmErrorResponse) {
	...
	conn := vtm.connector.getChildConnector("/tm/3.10/status/local_tm/backups/full/" + name)
	data, ok := conn.get()
	if ok != true {
...
func (vtm VirtualTrafficManager) ListSystemBackupsFull() (*[]string, *vtmErrorResponse) {
	conn := vtm.connector.getChildConnector("/tm/3.10/status/local_tm/backups/full")
	data, ok := conn.get()
	if ok != true {
		object := new(vtmErrorResponse)
		json.NewDecoder(data).Decode(object)
